...
def _estoque_submodulo_payload(cursor, modulo):
    ...
    ingredientes = _safe_lista_erp(cursor, """
        SELECT id, nome, unidade,
               ...
               CASE WHEN COALESCE(estoque_minimo,0) > 0 AND COALESCE(estoque_atual,0) <= COALESCE(estoque_minimo,0) THEN 'Baixo' ELSE 'OK' END AS status
        FROM ingredientes
    ...
    lotes = _safe_lista_erp(cursor, """
        SELECT
            e.id,
            COALESCE(pf.nome, e.produto_final_nome, e.receita_nome, 'Produto não vinculado') AS produto_nome,
            e.lote,
            e.lote AS codigo_lote,
            e.data_validade AS validade,
            COALESCE(e.saldo_atual, 0) AS saldo_atual,
            COALESCE(e.quantidade_produzida, e.saldo_atual, 0) AS quantidade,
            CASE
                WHEN pf.id IS NULL AND e.produto_final_id IS NOT NULL THEN 'Órfão'
                WHEN COALESCE(e.saldo_atual,0) > 0 THEN 'Com saldo'
                ELSE 'Sem saldo'
            END AS status,
            e.created_at
        FROM estoque_produto_acabado e
        LEFT JOIN produtos_finais pf ON pf.id = e.produto_final_id
        WHERE COALESCE(e.saldo_atual, 0) > 0
        ORDER BY COALESCE(e.data_validade, e.created_at) ASC, e.id DESC
        LIMIT 200
    """)
    if not lotes:
        lotes = _safe_lista_erp(cursor, """
    ...
    estoque_baixo_ing = [i for i in ingredientes if _safe_float_erp(i.get('estoque_minimo')) > 0 and _safe_float_erp(i.get('estoque_atual')) <= _safe_float_erp(i.get('estoque_minimo'))]
    estoque_baixo_prod = [p for p in produtos if _safe_float_erp(p.get('estoque_minimo')) > 0 and _safe_float_erp(p.get('estoque_atual')) <= _safe_float_erp(p.get('estoque_minimo'))]
